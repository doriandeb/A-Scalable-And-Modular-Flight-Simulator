...
Forces::Forces(string axisName, PlaneParameters p)
    ...
    position_earthFrame(p.position_earthFrame), // Renamed from displacement
    k(p.k),
    tailArea(p.tailArea),
...
void Forces::calcAerodynamicForce(PlaneParameters& p)
{
    ...
    double tas_xz = sqrt(velocity_bodyFrame.x * velocity_bodyFrame.x + velocity_bodyFrame.z * velocity_bodyFrame.z); 
...
void Forces::calcThrust(PlaneParameters& p, double dt) {
    ...
    // Find the targeted Thrust at steady state.
...
void updateRotationalDynamics(PlaneParameters& p, xyz torques, double dt) {
    ...
    // Safety check: Prevent divide-by-zero crash if pitched exactly 90 degrees (Gimbal lock)  (future improvements we can use quaternions)
    double psi_dot = 0.0;
